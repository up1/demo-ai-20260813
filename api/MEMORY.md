...
- The spec's 200 response example includes `"address"` in `data`, but the `users` DB schema table in the same spec doc has no `address` column and no endpoint ever writes one. Treated this as a real (if inconsistent) requirement rather than a doc typo: added `address VARCHAR(255) NOT NULL DEFAULT ''` to `migrations/0001_create_users_table.sql` and threaded `Address` through `domain.User` → repository → `usecase.LoginResult` → `loginData` JSON response. There is still no way to set a non-empty address (no register/update-profile endpoint in scope), so it will read back as `""` for any row inserted outside of tests. Revisit if a user-registration feature defines where `address` actually comes from.
